...
/* Dust sensor driver class */
GP2Y1010AU0F
dust(LED1, PC_5,
     PD_14); // PC_5 reads data from the sensor, PD_14 sends the Iled pulse
...
int main(int argc, char *argv[]) {
  // The purpose of the library is to provide a light, simple and general
  // tracing solution for mbed devices.
  mbed_trace_init(); // Allocates the trace buffer and initializes the internal
                     // variables.
  static unsigned int count = 0;
  bool isPeriodicSend = true;
  const float version = 1.0;
  static unsigned int id = 0;
  SocketAddress addr; // Due to newer mbed versions, a socketAddress object is
                      // needed for the connection
  NetworkInterface *network = NULL;

  Ticker periodicSensorRead;
  ...
  printf("Connected to the network successfully. MAC address: %s\r\n",
...
#endif
    addr.set_port(
        MQTT_SERVER_PORT); // Setting port and IP in the SocketAddress object
    network->gethostbyname(MQTT_SERVER_HOST_NAME, &addr);
    ret = socket->connect(addr); // Use the SocketAddress object instead of the
                                 // Hostname and string
    if (ret != NSAPI_ERROR_OK) {
  ...
  const char *username =
      MQTT_SERVER_HOST_NAME "/" DEVICE_ID "/api-version=2016-11-14";
  ...
  {
    int rc =
        mqttClient->subscribe(mqtt_topic_sub, MQTT::QOS0, handleMqttMessage);
    if (rc != MQTT::SUCCESS) {
 ...
 */
void handleButtonRise() { isPublish = true; }
